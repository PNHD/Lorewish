...
- **iOS build constraint** *(added by LW-M0-R2, and it is an M1 blocker)*: the owner's development
  machine is **Windows**. iOS builds cannot be produced locally on Windows — no Xcode, no local
  simulator, no local signing. Satisfying the ROADMAP M1 evidence bar ("app installs and runs on
  an iOS device") therefore requires **either** access to a Mac **or** a cloud build service
  (EAS Build), **plus** a paid Apple Developer Program membership for device installation beyond a
  simulator. Both carry real cost and owner setup time. This is the most likely cause of M1 quietly
  slipping to "Android and Web now, iOS later," which would silently break P9 — the commitment that
  all three platforms are first-class from the first slice. It must be resolved *before* M1 starts,
  not discovered during it. See [ROADMAP.md](ROADMAP.md) M1 prerequisites.
...
- **Regeneration reuses cached context assembly** but always counts as a new provider call against
  the allowance/ledger — no free unlimited retries, to prevent a cost-abuse loop.
