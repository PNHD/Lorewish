...
Open app
  ...
  → reach an ending / checkpoint
  → optionally: replay from an earlier branch point with a different choice
```
...
Scene rendered from canonical StoryState (current scene pointer + relevant canon facts/memories)
  ...
  → proposal is validated/applied: StoryState updated, new CanonFacts/Memories written if flagged,
    BranchHistory appended
  → scene re-rendered with the consequence
```
...
Create
  ...
  → other users play
  → branch/remix (fork the story into their own editable copy)
  → creator sees engagement (views, completions, branch count)
```
