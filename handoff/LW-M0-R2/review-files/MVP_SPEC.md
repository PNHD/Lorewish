...
2. **Story entry, two paths**:
   ...
   - **Advanced Setup**: structured form — premise, world/setting, genre, player role, main
     ...
     Narrative/Adventure), AI freedom (low/medium/high adherence to setup). See
     [UX_CONTRACT.md](UX_CONTRACT.md) for field-level behavior.
...
8. **Character chat surface**: standalone "Talk to Character" screen, character profile (identity
   ...
   character's profile.
9. **Branch/replay**: player can rewind to an earlier scene/checkpoint and choose differently,
   creating a new branch of their own run.
...
7. **Character Profile** — identity fields, speaking style, relationship to player, shared
   memories list.
8. **Branch/Replay Picker** — checkpoint list for the current run, "replay from here."
