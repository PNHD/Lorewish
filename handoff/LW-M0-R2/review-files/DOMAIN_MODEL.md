...
- **StoryConfiguration** — the Advanced Setup field set (world/setting, player role description,
  starting situation, tone, narrative POV, AI freedom) attached to a Story as its default. This is
  the template; a PlayerRun may hold its own override copy (see §2).
...
- **BranchHistory** — the record of which Branches exist for a PlayerRun and their fork points;
  drives the Branch/Replay Picker screen.
...
- Fields that **only affect future generation context** (tone, AI freedom, randomness level,
  world/setting flavor) can be changed freely; they're read fresh on the next GenerationProposal.
...
## 11. Explicitly Not Modeled in MVP
