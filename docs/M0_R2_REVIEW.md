...
Status: COMPLETE — **partly superseded by LW-M0-R3**
Task: **LW-M0-R2**
Date: 2026-08-10
Type: Independent review + documentation correction. **No implementation.**

> **Supersession notice (added by LW-M0-R3, 2026-08-10).** This document is a historical record of
> what the LW-M0-R2 session found, and its findings are left intact — they were accurate about the
> document set as it stood. Four items have since been resolved or corrected, and this notice
> exists so no reader acts on the stale versions:
>
> | Item in this document | Current status |
> |---|---|
> | §7.1, §9.5 — **P2 unreconstructable**, owner input required | **Resolved.** P2 is *Control Without Complexity*, supplied by the owner. See [DECISIONS.md](DECISIONS.md) D26 and [PRODUCT_VISION.md](PRODUCT_VISION.md) §9. No longer an M1 blocker. |
> | §7.2 — **"AI freedom" ruling pending** | **Resolved.** Cut from MVP; it never reaches a schema. D25. |
> | §7.3, §9.1, H9 — **iOS build path open; the most urgent question** | **Resolved.** Expo EAS cloud builds are the default iOS path from Windows; Apple Developer membership is a later prerequisite for device/TestFlight, not an M1 blocker. D24. |
> | §7.6, M9, §9.6 — **"three copies" of the Supabase skills; deduplicate to one** | **Corrected — this finding was wrong.** `.claude/skills/*` are NTFS **junctions** into `.agents/skills/*`, so there are two physical trees, not three, and 80 files, not ~120. The two trees differ only in `SKILL.md` frontmatter *rendering* for different runtime conventions. Acting on the original recommendation would have deleted the only real copy and left Claude Code with dangling junctions. Full inspection record: [AGENT_TOOLING.md](AGENT_TOOLING.md). |
>
> The overall M0 verdict has also moved on: this document concluded "PASS, conditional"; LW-M0-R3
> closes M0 as **PASS**. See `handoff/LW-M0-R3/HANDOFF.md`.
