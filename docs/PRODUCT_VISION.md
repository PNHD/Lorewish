...
Last updated: 2026-08-10 (revised by LW-M0-R2; P2 resolved and continuous play added by LW-M0-R3;
§10 UI-language row revised by LW-M1-R1 per D33)
...
**A world that remembers you is also a world that keeps going** *(added by LW-M0-R3)*. A world
which stops without warning after a choice does not read as one that remembers — it reads as one
that broke. The promise therefore carries a second, equally load-bearing half: the story stays
playable. Only a real ending ends it; a finished generation, a provider timeout, a submitted custom
action or a replayed branch never do. This is specified as a hard contract, not an aspiration, in
[CONTINUOUS_PLAY_CONTRACT.md](CONTINUOUS_PLAY_CONTRACT.md), and it is a direct response to
owner-observed evidence from an adjacent product
([REFERENCE_PRODUCT_NOTES.md](REFERENCE_PRODUCT_NOTES.md) §4.2).
...
> **Provenance note (added by LW-M0-R2, updated by LW-M0-R3).** The M0 document set referenced
> `P1`–`P10` more than thirty times across nine documents without ever defining them — every such
> reference resolved to nothing. The definitions below are **reconstructed from how each identifier
> is used** in those documents, so that the existing cross-references become resolvable. They are
> *inferred*, not quoted from an original brief — **with one exception: P2 is supplied verbatim by
> the product owner** (2026-08-10) and is therefore authoritative rather than reconstructed. If the
> owner holds the original text of any other principle, it supersedes this section.
...
| **P2** | **Control without complexity.** Two creation modes, both first-class: **Quick Start** for users who want to begin immediately with minimal setup, and **Advanced Setup** for users who want explicit control over world, role, characters and starting context. Neither mode is a degraded version of the other, and control is never imposed on a user who did not ask for it. *(Supplied by the product owner, 2026-08-10 — authoritative, not reconstructed.)* | [MVP_SPEC.md](MVP_SPEC.md) §1.2, [UX_CONTRACT.md](UX_CONTRACT.md) §4–5, §7 hypothesis 2 above, [DECISIONS.md](DECISIONS.md) D3, D26 |
...
|---|---|
| **Product positioning** | Global. **English and Vietnamese UI from the first implementation milestone** *(revised by LW-M1-R1, D33 — supersedes "English-first user interface")*. No region-specific product identity; Vietnamese is additive globalization, not a market pivot. |
| **Technical readiness** | i18n scaffolding **and both launch catalogues** from M1 (externalized strings, `en` + `vi` catalogues, locale-independent taxonomy keys, UTC-normalised timestamps, Unicode/IME-safe input). See [TECHNICAL_ARCHITECTURE.md](TECHNICAL_ARCHITECTURE.md) §12. |
| **Story content language** | A per-story property, independent of UI locale — a player may play in a language the UI does not yet ship. Generation is native-language-first, not translate-first (D34). See [DOMAIN_MODEL.md](DOMAIN_MODEL.md) §8 and [NARRATIVE_QUALITY_CONTRACT.md](NARRATIVE_QUALITY_CONTRACT.md). |
| **First test channel** | **Web** *(added by LW-M1-R1, D32)* — a URL is the fastest thing the owner can share; Android/iOS remain first-class and continue validating inside M1 on the same schedule. |
...
| **What the seed cohort does *not* decide** | Product-global scope, genre taxonomy, monetization design, data residency, or any architectural regionalisation. (It no longer decides UI language exclusivity either — both cohort-relevant languages ship.) |

The tension the original plan created is **materially reduced, not eliminated**: an English-only UI
tested on a cohort that is not natively English-speaking measured *language friction* alongside
product value. A Vietnamese-speaking tester now has a native-UI option, so the residual question
for [MVP_SPEC.md](MVP_SPEC.md) §5 Tier 4 review narrows to testers who use the English UI by choice
or default. Superseding [DECISIONS.md](DECISIONS.md) entries: **D2 (revised)**, **D33**.
