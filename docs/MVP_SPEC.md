...
removal by LW-M0-R3; §2 translated-UI exclusion revised by LW-M1-R1 per D33)
...
- 18+ content and associated moderation/age-gating (out of scope for the initial product entirely,
  not just MVP).
- **Translated UI beyond English and Vietnamese at launch** *(revised by LW-M1-R1, D33 — supersedes
  the original "beyond English" exclusion)*. English and Vietnamese UI catalogues both ship from
  M1, natively written per language rather than machine-translated (D34). Any *additional* language
  catalogue beyond those two remains excluded from MVP. The distinction sharpened by LW-M0-R2 still
  holds and now applies to two catalogues instead of one: *i18n scaffolding is in scope from M1*
  (externalized strings, locale-independent taxonomy keys, Unicode/IME-safe input), and shipping
  hardcoded UI copy is a violation of this spec regardless of which catalogues exist. See
