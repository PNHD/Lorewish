...
Last updated: 2026-08-10 (revised by LW-M0-R2; turn lifecycle and scene boundary by LW-M0-R3; §8
note added by LW-M1-R1 distinguishing UI locale from content_language now that UI is bilingual)
...
> **Note (LW-M1-R1, D33).** The UI itself is now bilingual (English + Vietnamese), which sharpens
> rather than removes the independence this section describes: `content_language` is still a
> per-Story property, orthogonal to whatever the player's *UI locale* is set to. All four
> combinations are valid — English UI / English story, English UI / Vietnamese story, Vietnamese UI
> / English story, Vietnamese UI / Vietnamese story. Generation is native-language-first per
> `content_language` regardless of UI locale (D34).
