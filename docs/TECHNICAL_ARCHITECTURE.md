...
LW-M0-R3; §11 revised by LW-M1-R1 per D33 — English + Vietnamese catalogues ship at M1, not just
scaffolding)
...
|---|---|---|
| **String externalization** — no user-facing copy hardcoded in components; **`en` and `vi` catalogues, both natively written, from M1** *(revised by LW-M1-R1, D33 — supersedes "one catalogue, English-only at launch")* | M1 | Retrofitting means touching every screen ever written. Adding a further locale later must be a data change, not a refactor. |
...
| **UTF-8 / IME-safe input end to end** | M1 (composer) | The composer is built in M1 and is the product's claimed differentiator (P7). See [UX_CONTRACT.md](UX_CONTRACT.md) §2. Vietnamese diacritic input/rendering is now a first-party UI language, not only a story-content case. |
...
| **Story `content_language` on the Story record, native-language-first generation** | M2 | Prompt construction depends on it from the first generated scene ([DOMAIN_MODEL.md](DOMAIN_MODEL.md) §8, D34, [NARRATIVE_QUALITY_CONTRACT.md](NARRATIVE_QUALITY_CONTRACT.md)). |
...
What is **not** required at M1: translated string catalogues **beyond English and Vietnamese**,
right-to-left layout support, currency handling, or region-specific content policy. Those remain M7
concerns (or, for further-language catalogues, an unscheduled future decision). The distinction is
still **scaffolding versus catalogues** — M1 now ships two catalogues instead of one because the
owner decided the second catalogue is cheap at scaffold time and expensive to retrofit, the same
logic this table already used to justify everything else in it.
